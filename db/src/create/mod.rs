...
pub async fn create_class(database: &D1Database, name: &str, description: &str, capacity: i32, code: &str, type_: &str, section: &str, term: &str, year: &String, credit_hrs: &i32) -> std::result::Result<(), CreateError> {
    execute_insert_query(database, "classes", &["name", "description", "capacity", "code", "class_type", "section", "term", "year", "credit_hrs"], &[name, description, &capacity.to_string(), code, type_, section, term, year, &credit_hrs.to_string()]).await
}
